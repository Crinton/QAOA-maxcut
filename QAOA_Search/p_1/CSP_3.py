...
def QaoaGetPara(gamma,beta,t,isOutStat = False):
    ...
    A = _Rx(0,4,beta)*_Rx(1,4,beta)*_Rx(2,4,beta)*_Rx(3,4,beta) \
        ...
        *_Rzz(0,1,4,gamma)*_Rzz(2,3,4,gamma)*_Rzz(0,2,4,gamma)*(_Rzz(1,3,4,-gamma)) \
    ...
    H_C = -np.kron(z,I(3)) + np.kron(np.kron(I(1),z),I(2)) + np.kron(np.kron(I(2),z),I(1))  \
          ...
          +np.kron(np.kron(np.kron(z,I(1)),z),I(1)) - np.kron(np.kron(np.kron(I(1),z),I(1)),z)
...
if __name__ == "__main__":
    ...
    plt.savefig("heatmap-CSP_3.jpg", bbox_inches = "tight")
    # 创建狄拉克符号的基态字符串列表
    ...
    plt.savefig("bar-CSP_3.jpg", bbox_inches = "tight")
